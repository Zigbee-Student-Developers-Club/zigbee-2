...
export const POST = async (req: NextRequest) => {
    try {
        const { batch, phoneNumber, name, linkedInUrl, position, profileImg, domain, about } = await req.json();

        if (!batch || !name || !linkedInUrl || !profileImg || !domain) {
            return NextResponse.json(
                {
                    error: "Batch, Name, LinkedIn URL, Profile Image, and Domain fields are required.",
                    success: false,
                },
                { status: 400 }
            );
